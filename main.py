...
def download_from_gdrive(file_id: str, dest: str) -> None:
    ...
    # Use shutil.move instead of os.replace — more reliable across filesystems
    import shutil
    shutil.move(tmp_dest, dest)
    print(f"Model saved to {dest} ({size_mb:.1f} MB)")


# ── Blocking load (runs in thread executor) ────────────────────────────────────
def _load_model_blocking() -> None:
    global model
    download_from_gdrive(GDRIVE_FILE_ID, LOCAL_MODEL_PATH)

    # Double-check file exists before TF tries to open it
...
async def lifespan(app: FastAPI):
    try:
        # Run blocking download + TF load in a thread — keeps event loop free
        # and ensures startup completes BEFORE uvicorn accepts requests
