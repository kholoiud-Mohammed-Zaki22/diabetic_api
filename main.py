...
# ── Constants ──────────────────────────────────────────────────────────────────
IMAGE_SIZE       = (224, 224)
CLASS_NAMES      = ["Non_Ulcer", "Ulcer"]   # alphabetical — matches training
LOCAL_MODEL_PATH = "/tmp/diabetic_foot_uIcer.keras"
MAX_UPLOAD_MB    = 10
GDRIVE_FILE_ID   = os.getenv("GDRIVE_FILE_ID", "1RDeUOJ7lpF41yjPZliBdJmV0u3K75YdG")
...
def download_from_gdrive(file_id: str, dest: str) -> None:
    ...
    Uses a .tmp file to avoid leaving a corrupt file on disk if anything fails.
    """
    import gdown

    # Skip only if cached file is valid (> 1 MB)
    ...
    # Remove stale .tmp if it exists from a previous failed attempt
    if os.path.exists(tmp_dest):
        os.remove(tmp_dest)

    try:
        result = gdown.download(url, tmp_dest, quiet=False, fuzzy=True)
    except Exception as exc:
        if os.path.exists(tmp_dest):
            os.remove(tmp_dest)
        raise RuntimeError(f"gdown raised an exception: {exc}") from exc

    # gdown returns None on failure instead of raising
    if result is None or not os.path.exists(tmp_dest):
        raise RuntimeError(
            "gdown returned None — download failed. "
            "Check that the file is shared as 'Anyone with the link'."
        )

    # Reject HTML error pages (a real keras model is never < 1 MB)
    size_mb = os.path.getsize(tmp_dest) / 1e6
    if size_mb < 1:
        os.remove(tmp_dest)
        raise RuntimeError(
    ...
    os.replace(tmp_dest, dest)  # atomic on Linux
    print(f"Model saved to {dest} ({size_mb:.1f} MB)")
...
def preprocess_image(image_bytes: bytes) -> np.ndarray:
    ...
    NO manual /255 — EfficientNetB0 includes its own rescaling.
    """
...
async def predict(file: UploadFile = File(...)):
    ...
    try:
        with model_lock:
            raw_output = model.predict(img_array, verbose=0)   # (1, 2)
    except Exception as e:
